...
public protocol DataReaderProtocol {
    ...
    mutating func readByte(advance: Bool) throws(DataReaderError) -> DataElement
    ...
    mutating func read(bytes count: Int?, advance: Bool) throws(DataReaderError) -> DataRange
...
extension DataReaderProtocol {
    ...
    ///
    /// If fewer bytes remain than are requested, an error will be returned.
    public mutating func read(advance: Bool) throws(DataReaderError) -> DataRange {
...
extension DataReaderProtocol {
    ...
    public mutating func nonAdvancingReadByte() throws(DataReaderError) -> DataElement {
        try readByte(advance: false)
    }
    
    @_documentation(visibility: internal)
    @available(*, renamed: "read(advance:)")
    public mutating func nonAdvancingRead() throws(DataReaderError) -> DataRange {
        try read(advance: false)
    }
    
    @_documentation(visibility: internal)
    @available(*, renamed: "read(bytes:advance:)")
    public mutating func nonAdvancingRead(bytes count: Int) throws(DataReaderError) -> DataRange {
