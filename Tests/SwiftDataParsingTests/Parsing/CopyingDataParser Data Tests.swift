//
//  CopyingDataParser Data Tests.swift
...
@Suite
struct CopyingDataParser_Data_Tests {
    ...
    func nonAdvancingRead_DataIndicesOffset() throws {
        ...
        // non-advancing read - read overflow - return nil
        data.withCopyingDataParser { parser in
            #expect(throws: (any Error).self) { try parser.read(bytes: 8, advance: false) }
        }
    }
    ...
    @Test
    func withCopyingDataParser() throws {
        let data = Data([0x01, 0x02, 0x03, 0x04])
    ...
    @Test
    func withCopyingDataParser_ReturnsValue() throws {
        let data = Data([0x01, 0x02, 0x03, 0x04])
