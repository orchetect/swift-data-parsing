//
//  PointerDataParser Data Tests.swift
...
@Suite
struct PointerDataParser_Data_Tests {
    ...
    func nonAdvancingRead() throws {
        ...
        // single bytes
        try data.withPointerDataParser { parser in
            try #expect(parser.readByte(advance: false) == data[0])
            try #expect(parser.readByte(advance: false) == data[0])
            try #expect(parser.readByte() == data[0])
    ...
    @Test
    func seekBy_DataIndicesOffset() throws {
        let rawData = Data([0x00, 0x00, 0x00, 0x01, 0x02, 0x03, 0x04])
        let data = rawData[3 ... 6]
    ...
    @Test
    func withPointerDataParser() throws {
        let data = Data([0x01, 0x02, 0x03, 0x04])
    ...
    @Test
    func withPointerDataParser_ReturnsValue() throws {
        let data = Data([0x01, 0x02, 0x03, 0x04])
